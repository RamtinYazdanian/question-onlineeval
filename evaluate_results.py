...
def convert_json_to_results(json_file):
    ...
    for question_type_name in QUESTION_GROUP_TYPES:
        ...
        subfields = [order_q_based_first([comparison_pair_names[x], subfields[x]]) for x in range(N_PAIRS)]
...
def get_all_results(list_of_files):
    ...
    for filename in list_of_files:
        result_list.append(convert_json_to_results(filename))
    return result_list
...
def get_descriptive_stats(df):
    ...
    print(df.groupby('name').apply(lambda x: dict(Counter(list(x)))))
